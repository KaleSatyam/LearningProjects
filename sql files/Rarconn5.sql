...
from xx_sshr_loan_from_saas where person_id=100000007457225 and OUTSTANDING_AMT>0;
 ...
 where sa.person_id=100000007457661 and OUTSTANDING_AMT>0 and SOURCESYSTEMOWNER='PAAS' and to_number(SUBSTR(LOAN_ID,6,length(LOAN_ID)-19))
 in(select SSHR_REQ_MASTER_ID from xx_sshr_req_master);
 
 select to_number(SUBSTR(LOAN_ID,6,length(LOAN_ID)-19)) as saas_loan_id from xx_sshr_loan_from_saas sa inner join xx_sshr_req_master req
 on to_number(SUBSTR(sa.LOAN_ID,6,length(sa.LOAN_ID)-19))=req.SSHR_REQ_MASTER_ID
 where sa.person_id=100000007457661 and OUTSTANDING_AMT>0 and SOURCESYSTEMOWNER='PAAS';
 
 select * from xx_sshr_req_master;
 select * from xx_sshr_loan_from_saas;
select * from xx_sshr_req_master where ref_id like '%DJ%';
select person_id,count(*) from xx_sshr_loan_from_saas where outstanding_amt>0 and SOURCESYSTEMOWNER='PAAS'  group by person_id having COUNT(*)>1;
select * from xx_sshr_loan_from_saas where person_id=100000007445766;
